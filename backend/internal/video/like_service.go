...
func NewLikeService(repo *LikeRepository, videoRepo *VideoRepository, cache *rediscache.Client, likeMQ *rabbitmq.LikeMQ, popularityMQ *rabbitmq.PopularityMQ, localCacheMQ *rabbitmq.LocalCacheMQ) *LikeService {
	return &LikeService{repo: repo, VideoRepo: videoRepo, cache: cache, likeMQ: likeMQ, popularityMQ: popularityMQ, localCacheMQ: localCacheMQ}
}
...
func (s *LikeService) Like(ctx context.Context, like *Like) error {
	...
	PublishVideoCacheInvalidation(ctx, s.localCacheMQ, like.VideoID)
	return nil
}
