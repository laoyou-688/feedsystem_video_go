...
func main() {
	...
	rmq := &rabbitmq.RabbitMQ{Conn: conn, Ch: ch}
	localCacheMQ, err := rabbitmq.NewLocalCacheMQ(rmq)
	if err != nil {
		log.Printf("LocalCacheMQ init failed (worker publish disabled): %v", err)
		localCacheMQ = nil
	}

	socialWorker := worker.NewSocialWorker(ch, cache, socialQueue)
	...
	likeWorker := worker.NewLikeWorker(ch, cache, likeRepo, videoRepo, likeQueue, localCacheMQ)
	commentWorker := worker.NewCommentWorker(ch, cache, commentRepo, videoRepo, commentQueue, localCacheMQ)
