...
const latest = reactive<ListState & { limit: number; next_time: number; next_id_before?: number; sort_mode: 'latest' | 'hybrid' }>({
  loading: false,
  error: '',
  items: [],
  has_more: false,
...
const following = reactive<ListState & { limit: number; next_time: number; next_id_before?: number }>({
  loading: false,
  error: '',
  items: [],
  has_more: false,
...
async function loadLatest(reset: boolean) {
  ...
  try {
    ...
    const latest_id_before = reset ? undefined : latest.next_id_before
    const res = await feedApi.listLatest({ limit: latest.limit, latest_time, latest_id_before, sort_mode: latest.sort_mode })
    latest.has_more = res.has_more
    latest.next_time = res.next_time
    latest.next_id_before = res.next_id_before
...
async function loadFollowing(reset: boolean) {
  ...
  try {
    ...
    const latest_id_before = reset ? undefined : following.next_id_before
    const res = await feedApi.listByFollowing({ limit: following.limit, latest_time, latest_id_before })
    following.has_more = res.has_more
    following.next_time = res.next_time
    following.next_id_before = res.next_id_before
        ...
        <div class="card" style="margin-top: 12px">
          <div class="row" style="justify-content: space-between">
            <div>
              ...
              <div class="subtle">limit：{{ latest.limit }} · mode：{{ latest.sort_mode }} · next_time：{{ latest.next_time }} · has_more：{{ latest.has_more }}</div>
            </div>
            <div class="row">
              <label class="subtle" style="margin: 0">mode</label>
              <select v-model="latest.sort_mode" style="width: 110px">
