...
const recommend = reactive({
  ...
  sortMode: 'hybrid' as 'latest' | 'hybrid',
})
...
async function loadRecommend(reset: boolean) {
  ...
  recommend.error = ''
  try {
    const res = await feedApi.listLatest({
      limit: 10,
      latest_time: reset ? 0 : recommend.nextTime,
      latest_id_before: reset ? undefined : recommend.nextIdBefore,
      sort_mode: recommend.sortMode,
    })
    recommend.hasMore = res.has_more
    recommend.nextTime = res.next_time
    recommend.nextIdBefore = res.next_id_before
...
async function loadFollowing(reset: boolean) {
  ...
  try {
    const res = await feedApi.listByFollowing({
      ...
      latest_id_before: reset ? undefined : following.nextIdBefore,
    })
    following.hasMore = res.has_more
    following.nextTime = res.next_time
    following.nextIdBefore = res.next_id_before
...
<template>
  <AppShell full>
    <div class="page">
      <div class="tabs">
        <button class="tab" :class="{ on: tab === 'recommend' }" type="button" @click="tab = 'recommend'">
          推荐{{ tab === 'recommend' ? ` · ${recommend.sortMode}` : '' }}
        </button>
        ...
        <div class="tabs-right">
          <template v-if="tab === 'recommend'">
            <button class="chip" type="button" :class="{ primary: recommend.sortMode === 'latest' }" @click="recommend.sortMode = 'latest'; loadRecommend(true)">
              时间排序
            </button>
            <button class="chip" type="button" :class="{ primary: recommend.sortMode === 'hybrid' }" @click="recommend.sortMode = 'hybrid'; loadRecommend(true)">
              混合排序
            </button>
          </template>
